    ...
    <section id="user-input">
      <div className="input-group">
        <p>
          ...
          /* And we can do that with help of the value prop. This prop can be
          used to set the value that will be displayed on the screen for this
          input field. */
          <input
            type="number"
            required
          ...
          />
          /* We can add the onChange prop to add a listener to the change event
          to this input field. And you can then define which function should get
          executed when that change event occurs. Now just pointing at handle
          change like this, so at this handle change function here won't work
          though because by default when the change event occurs, React will
          call this function and pass the event object to it. A default event
          object that is automatically generated by React. But whilst this event
          object will contain useful data, it will not hold the inputIdentifier
          and it will also just be one parameter instead of two, which we're
          expecting here. By applying anonymous function here, which is then the
          actual function that's passed as a value to onChange. And that will be
          executed by React eventually once the change event occurs and inside
          of this function we therefore then have full control over how handle
          change will eventually be executed. Because it is this anonymous
          function that is passed as a value to onChange, and that will be
          called by React, therefore. it will get this event object and it has a
          target property which will refer to this input. */
        </p>
        <p>
